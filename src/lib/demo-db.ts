...
const globalForDb = globalThis as typeof globalThis & {
  __crewSyncDemo?: { clients: Client[]; counter: number };
};

function getStore(): { clients: Client[]; counter: number } {
  if (!globalForDb.__crewSyncDemo) {
    globalForDb.__crewSyncDemo = { clients: [], counter: 1 };
  }
  return globalForDb.__crewSyncDemo;
}
